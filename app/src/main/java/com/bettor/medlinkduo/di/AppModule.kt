...
import com.bettor.medlinkduo.core.feedback.SensoryFeedback
import com.bettor.medlinkduo.data.ble.MockBleRepository
import com.bettor.medlinkduo.data.local.AppDatabase
import com.bettor.medlinkduo.data.store.OnboardingStore
...
object AppModule {
    ...
    @Provides @Singleton
    fun bleRepo(@ApplicationContext ctx: Context, scope: CoroutineScope): BleRepository =
        MockBleRepository(ctx, scope)

    @Provides @Singleton
    fun provideTts(@ApplicationContext ctx: Context): TtsController =
        AndroidTtsController(ctx)

    @Provides @Singleton
    fun db(@ApplicationContext ctx: Context): AppDatabase =
        Room.databaseBuilder(ctx, AppDatabase::class.java, "app.db").build()

    @Provides fun measurementDao(db: AppDatabase) = db.measurementDao()
    @Provides fun feedbackDao(db: AppDatabase) = db.feedbackDao()
    @Provides @Singleton fun sensory(@ApplicationContext ctx: Context) = SensoryFeedback(ctx)
}
