...
fun HapticFeedback.play(event: HapticEvent, platform: PlatformHaptics? = null) {
    when (event) {
        HapticEvent.ScanStart,
        HapticEvent.SafeStop -> performHapticFeedback(HapticFeedbackType.LongPress)

        HapticEvent.ScanDone,
        HapticEvent.ReRead   -> performHapticFeedback(HapticFeedbackType.TextHandleMove)
