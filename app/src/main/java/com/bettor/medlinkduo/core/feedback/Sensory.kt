package com.bettor.medlinkduo.core.feedback
...
class SensoryFeedback(private val ctx: Context) {
    ...
    fun success() { tone.startTone(ToneGenerator.TONE_PROP_BEEP2, 120) }
    fun error() { tone.startTone(ToneGenerator.TONE_SUP_ERROR, 200) }
    ...
    fun vibrate(ms: Long = 40) {
        ...
        if (Build.VERSION.SDK_INT >= 26) vib.vibrate(VibrationEffect.createOneShot(ms, 160))
